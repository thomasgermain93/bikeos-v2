...
const SPRINT_COLOR = '#f97316'; // Orange pour les sprints
...
export function SprintCard({ race, sprintResults }: SprintCardProps) {
  const raceDate = new Date(race.date);
  
  // Top 5 des résultats de sprint
  const results = sprintResults.slice(0, 5);
  // Points pour sprint: 12, 9, 7, 6, 5
  const pointsMap = [12, 9, 7, 6, 5];
  ...
  return (
    <div className="border border-[var(--border-card)] rounded-xl bg-zinc-900 overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-1.5">
          <div className="flex items-center gap-2">
            <span 
              ...
              style={{ backgroundColor: SPRINT_COLOR }}
            ></span>
            <span 
              className="text-xs font-mono font-medium uppercase tracking-widest px-1.5 py-0.5 rounded"
              style={{ color: SPRINT_COLOR, backgroundColor: `${SPRINT_COLOR}18` }}
            >
              SPRINT
            </span>
          </div>
      ...
      <div>
        ...
        {/* Fastest Lap - affiché seulement si disponible */}
        {results[0] && (
